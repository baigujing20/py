...
const DEFAULT_UUID = '5df5b7d4-e767-45d0-8626-2c2092432088';
...
async function parseClientPacket(buffer, cfg) {
  ...
  const addr = parseVlessAddress(bytes, dv, addrType, 22 + optLen);
  return {
    proto: 'vless',
    command,
    address: addr.host,
...
function parseTrojan(bytes, dv) {
  // Trojan 的 ATYP 使用 SOCKS5 编码：
  // 0x01 IPv4 / 0x03 域名 / 0x04 IPv6。
  // 解析后统一返回与 VLESS 相同的对象，供转发逻辑共用。
  const cmd = bytes[58]; // 0x01 CONNECT / 0x03 UDP_ASSOCIATE
  if (cmd !== 1 && cmd !== 3) {
    throw new Error('不支持的 Trojan 命令: ' + cmd);
  }

  const addr = parseTrojanAddress(bytes, dv, bytes[59], 60);
  const portOffset = addr.offset;

  // 端口 2 字节 + 结尾 CRLF 2 字节。
  if (portOffset + 4 > bytes.length) {
    throw new Error('Trojan 首包过短');
  }

  const port = dv.getUint16(portOffset);
  const payloadOffset = portOffset + 2;

  if (bytes[payloadOffset] !== 13 || bytes[payloadOffset + 1] !== 10) {
    throw new Error('Trojan 首包缺少 CRLF 分隔符');
  }
  ...
  return {
    ...
    address: addr.host,
    port,
    payload: bytes.slice(payloadOffset + 2),
    responseHeader: new Uint8Array(0),
  };
}

function parseVlessAddress(bytes, dv, addrType, start) {
  // VLESS: 0x01 IPv4 / 0x02 域名 / 0x03 IPv6
  return parseAddress(
    bytes,
    dv,
    addrType,
...
function parseTrojanAddress(bytes, dv, addrType, start) {
  // Trojan/SOCKS5: 0x01 IPv4 / 0x03 域名 / 0x04 IPv6
  return parseAddress(
    bytes,
    dv,
    addrType,
...
function parseAddress(bytes, dv, addrType, start, types, protocol) {
  if (addrType === 1) {
    if (start + 4 > bytes.length) {
      throw new Error(protocol + ' IPv4 地址不完整');
    }
    return {
      host: Array.from(bytes.slice(start, start + 4)).join('.'),
      offset: start + 4,
    };
  }

  if (addrType === types.domain) {
    if (start >= bytes.length) {
      throw new Error(protocol + ' 域名长度缺失');
    }

    const len = bytes[start];
    if (start + 1 + len > bytes.length) {
      throw new Error(protocol + ' 域名地址不完整');
    }

    const host = new TextDecoder().decode(
      bytes.slice(start + 1, start + 1 + len)
    );
    return {
      host,
      offset: start + 1 + len,
  ...
  if (addrType === types.ipv6) {
    if (start + 16 > bytes.length) {
      throw new Error(protocol + ' IPv6 地址不完整');
    }

    const parts = [];
    for (let i = 0; i < 8; i++) {
      parts.push(dv.getUint16(start + i * 2).toString(16));
    }
    return {
      host: parts.join(':'),
      offset: start + 16,
  ...
  throw new Error(protocol + ' 不支持的地址类型: ' + addrType);
}
// ============================ 节点 / 订阅生成 ============================
