...
if __name__ == '__main__':
    ...
    endpoints = read_csv_endpoints(files[1])
    # 获取*.conf的参数值
    ...
    addr = re.sub(r"/\d+", "", ','.join(address).replace(' ', ''))
    ...
    for i, endpoint in enumerate(endpoints):
        ...
        v2rayn_link = f'wireguard://{quote(prik, safe="")}@{ip}:{port}?&publickey={quote(pubk, safe="")}&reserved={quote(rsvd, safe="")}&address={quote(addr, safe="")}&mtu={mtu}#{quote(remarks, safe="")}'
        f.write(f"{v2rayn_link}\n")
        f.flush()
